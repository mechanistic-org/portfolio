...
export function careerTimelineRecords(nodes: readonly CareerNode[]) {
	return nodes.flatMap((node) => {
		if (node.type !== "project" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(node.id)) return [];
		const chronology = projectPeriod(node.id, node.data);
		const start = timestamp(chronology.start);
		if (start === null || typeof node.data.title !== "string" || !node.data.title.trim()) return [];
		const candidateEnd = timestamp(chronology.end);
		...
		return [
			{
				...
				basis: chronology.basis,
				context: chronology.context,
				start,
				end,
...
export function buildContextRibbon(
	...
	const records = careerTimelineRecords(nodes);
