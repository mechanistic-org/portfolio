...
- **Self-Correction:** If a layout change fails and is reverted, document _why_ it failed in `STYLE_GUIDE.md` to prevent future loops.
  ...
  > - **Asset Hygiene:** **NEVER COPY** large assets into the `ErikNorris` repo to fix 404s. Check the Symlink logic first. If the file is in `ErikNorris-assets` but missing on localhost, it's a symlink issue, not a missing file. Trust the Air Gap.
...
> **PROTOCOL: ASSET SOVEREIGNTY (THE ErikNorris LAWS):**
>
> 1.  **Law of Continuity (Truth):** Existing assets (Historical Truth) must be preserved. A 404 on an existing project ID is a **failure of retrieval**, not an absence of existence. _Action:_ If a historical asset matches a live URL but fails locally, debug the Symlink/Path first.
...
> 4.  **Ready**: Only _after_ these steps, reply: "Erik Norris Portfolio Online. Ready for instructions."
...
> 3.  `src/content/docs/MAINTENANCE.md` (System Manual) - _See "Ghost Port Anomaly"_
> 4.  `src/content/docs/ROADMAP.md` (Current Status)
> 5.  `src/content/docs/manifesto.md` (Design Philosophy)
>
> **NEW WORKFLOWS:**
>
...
> - [`UNIVERSAL_NOTEBOOK_PROMPT.md`](/src/content/docs/prompts/UNIVERSAL_NOTEBOOK_PROMPT.md): The forensic engineering parser for analyzing project notebooks.
> - [`CONVERSATION_MINER_PROMPT.md`](/src/content/docs/prompts/CONVERSATION_MINER_PROMPT.md): The protocol for extracting "Gold" from chat logs.
>
