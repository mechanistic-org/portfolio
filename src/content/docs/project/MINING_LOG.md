...
- **[TECH] Cloudflare WAF Bypass:** Engineered a permanent VIP Tunnel in Cloudflare (URI Path equals `/llms.txt` -> Skip All Managed Rules) to bypass the JS CAPTCHA Challenge for headless LLM scrapers.
- **[TECH] Agentic Breadcrumbs:** Injected the `<link rel="alternate" type="text/plain" href="/llms.txt">` absolute path into `BaseHead.astro` to ensure bots scanning the root domain are immediately routed to the machine-readable payload.
- **[META] Semantic Projection Analysis:** Validated that dense structural prompts execute "Contextual Mirroring." An LLM will adopt proprietary taxonomy and generate logical essays even if physically blocked by WAF.
