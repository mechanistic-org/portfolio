...
* **Key Decision (Metrics):** **"The Tactile Volume Bar"** - "Status Label" (Text) is deprecated. Replaced by "Production Scale" (Visualize 1-5 bars).
* **Key Decision (Deployment):** **"The KV Binding Trap"** - When building for Production (Static), you MUST remove `[[kv_namespaces]]` from `wrangler.toml` or the build will fail. Local (Server) adapters handle this automatically.
