...
- label: Supply chain — the top-panel "no-bid"
  ...
  result: 'Unblocked Pilot and protected the delivered November 7, 2007 First Customer Ship. (Human cost: a fully ruptured Achilles mid-fab — weld-stud files sent from hotel wifi so the build would not stall.)'
- label: Serviceability — the headphone-jack fire drill
  ...
  intervention: 'Emergency redesign past Tooling-Control-Off: a recessed service geometry initiated April 25; the urgent July 12 change order is internally numbered ECO 12993 but archived under ECO_12996. The conflicting part fields are not used to infer affected parts.'
...
- **Trigger:** Mid-schedule, the primary overseas CM (Kwanta) formally no-bid the console's most complex part — the Top Panel (P/N 9420-55105). Its extreme density of welded standoffs physically blocked the vendor's automated CNC welding heads, threatening a line-down scenario. The earliest located primary checkpoint is the March 7, 2007 status report, which records the no-bid and the active Operations/Manufacturing response; the April 4 report says the matter was then before Kwanta's board and documents the Mass Precision/Kenny bridge options. (Vendor structure, per the archives + operator rulings 2026-07-12: VTech Communications Ltd [CMS division, Dongguan] = the overarching CM for assembly and test; Kwanta = VTech's metal partner and the entity that no-bid; Kenny [Kenny Products] = a VTech subsidiary and the secondary metal vendor; KwanAsia [Macao] = early kitting and welding-capacity interface; Quanta = a distinct bidder per operator recollection, not distinguishable in the surviving records.)
...
- **Result:** Unblocked the Pilot build and transitioned cleanly from domestic manual parts to overseas automated lines, protecting the delivered November 7, 2007 First Customer Ship date.
...
- **Intervention:** Executed an emergency redesign past the "Tooling Control Off" milestone, developing a recessed service geometry from the April 25 escalation. The urgent July 12 change order is internally numbered ECO 12993 but archived under `ECO_12996`; its part-level fields conflict with the email record, so the chronology preserves the mounting-change claim without inferring affected part numbers.
...
- **Reworked** 150 Rev C Monitor PCBs (9150-55199) by hand to kill a talkback audio-bleed defect. The verified delivery record places First Customer Ship on November 7, 2007; the earlier November 20 date was the Phase 1 plan, not the delivered milestone.
