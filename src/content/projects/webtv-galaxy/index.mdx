...
metrics:
  ...
  war_stories:
  - description: Forced chassis growth to 300mm and integrated 50x60x30mm extruded
      heatsink to prevent component meltdown.
    label: The 150-Watt Thermal Crisis
    value: 28.3 CFM / 84°C Tj
  - description: Redesigned stamping features from drawn standoffs to 'Hook 29' details
      to arrest progressive die warpage.
    label: The Warped Base Defect
    value: 100% Flatness Yield
  - description: Decomposed the CPU enclosure into sub-components to resolve fastener
      interference with the fixed air partition.
    label: The Impossible Assembly
    value: Modular In-Situ Build
  ...
  - description: The product didn't ship, but the DNA survived. The wind tunnel architecture
      and thermal learnings were directly absorbed by the Xbox team. We built the
      one that broke to teach them how to build the one that didn't.
    label: Reincarnation
