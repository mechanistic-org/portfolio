...
export default function HydePeriodicTable({ width = 800, height = 800 }: HydePeriodicTableProps) {
    ...
    return (
        <div className="relative w-full aspect-square flex items-center justify-center bg-neutral-950 rounded-3xl overflow-hidden border border-white/5 shadow-2xl">

            {/* Background Grid/Glow */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(46,92,255,0.1)_0%,rgba(0,0,0,0)_70%)]" />

            <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="z-10 w-full h-full max-w-[800px] max-h-[800px]">
                ...
                {nodes.map((node, i) => (
                    ...
                    >
                        ...
                        <circle
                            ...
                            strokeWidth="1"
                            className="transition-all duration-300 group-hover:fill-opacity-100 group-hover:scale-125"
                        />
                        ...
                        {(i < 10 || hoveredTrait?.name === node.name) && (
                            <text
                                ...
                                fontSize="10"
                                fontWeight="bold"
                                className="pointer-events-none drop-shadow-md tracking-wider uppercase"
                            >
                                {node.name}
                            </text>
                        )}
            ...
            <AnimatePresence>
                {hoveredTrait && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 20 }}
                        className="absolute bottom-8 right-8 bg-neutral-900/90 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-2xl z-20 min-w-[200px]"
                    >
                        <div className="text-xs font-mono text-neutral-500 mb-1">{hoveredTrait.category}</div>
                        <div className="text-xl font-bold text-white mb-2">{hoveredTrait.name}</div>
