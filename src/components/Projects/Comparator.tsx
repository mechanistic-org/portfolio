...
export default function Comparator({
	...
	return (
		<div
			ref={containerRef}
			className="group relative aspect-video h-full max-h-[80vh] w-full overflow-hidden border border-white/10 bg-neutral-900 select-none"
		>
			...
			<img
				...
				draggable={false}
			/>
			<div className="absolute top-4 right-4 border border-green-500/30 bg-black/50 px-2 py-1 font-mono text-xs text-green-400 backdrop-blur-md">
				{afterLabel}
			</div>
			...
			>
				...
				<motion.img
					...
					style={{ width: width }}
				/>
				<div className="absolute top-4 left-4 z-10 border border-red-500/30 bg-black/50 px-2 py-1 font-mono text-xs text-red-400 backdrop-blur-md">
					{beforeLabel}
				</div>
			</motion.div>
			...
			{/* HANDLER */}
			<motion.div
				className="absolute top-0 bottom-0 z-20 flex w-1 cursor-ew-resize items-center justify-center bg-white"
				style={{ x }}
				drag="x"
			...
			>
				<div className="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-[0_0_20px_rgba(0,0,0,0.5)]">
					<svg
						width="12"
						height="12"
						viewBox="0 0 24 24"
