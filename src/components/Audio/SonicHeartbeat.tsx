...
const SonicHeartbeat: React.FC<SonicHeartbeatProps> = ({ audioUrl }) => {
	...
	useEffect(() => {
		...
		if (animPhase === "ecg") {
			timeout = setTimeout(() => setAnimPhase("flash"), 1500);
		} else {
			timeout = setTimeout(() => setAnimPhase("ecg"), 1500);
		}
	...
	return (
		...
		>
			<div
				className={`relative h-8 w-16 overflow-hidden rounded-sm border bg-black/40 backdrop-blur-sm transition-colors duration-300 ${
					isPlaying
							...
							<>
								...
								{animPhase === "ecg" ? (
									...
									<motion.path
										...
										transition={{ opacity: { duration: 0.5, delay: 0.5, ease: "easeOut" } }}
									/>
								)}
							</>
						) : (
