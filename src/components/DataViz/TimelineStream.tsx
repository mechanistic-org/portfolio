...
const TimelineStream: React.FC<TimelineStreamProps> = ({ data }) => {
	...
	useEffect(() => {
		...
		const simulation = d3
			...
			.force(
				...
				d3
					.forceCollide()
					.radius((d: any) => d.value * 0.8 + 1)
					.iterations(2),
			) // Less padding than bubbles
	...
	return (
		<div ref={wrapperRef} className="relative h-full w-full" style={{ minHeight: "600px" }}>
			<svg ref={svgRef} width="100%" height="100%" className="overflow-visible" />
			{selectedNode && (
				<div className="pointer-events-none absolute top-4 left-4 rounded-lg border border-white/10 bg-black/80 p-4 shadow-xl backdrop-blur">
					<div className="font-bold text-white">{selectedNode.name}</div>
					<div className="font-mono text-xs text-[#2E5CFF]">
