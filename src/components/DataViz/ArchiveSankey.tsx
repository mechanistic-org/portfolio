...
const ArchiveSankey: React.FC<ArchiveSankeyProps> = ({ nodes: rawNodes }) => {
			...
			const existingLink = links.find(
				(l) =>
					(typeof l.source === "number"
						? l.source === catIdx
						: (l.source as any) === nodes[catIdx].id) &&
					(typeof l.target === "number"
						? l.target === compIdx
						: (l.target as any) === nodes[compIdx].id),
			);
	...
	useEffect(() => {
		...
		linkLayer
			...
			.on("mouseover", function () {
				d3.select(this)
					.attr("opacity", 0.8)
					.attr("stroke-width", (d: any) => Math.max(1, (d.width || 1) + 2));
			})
	...
	return (
		<div ref={containerRef} className="relative h-full min-h-[500px] w-full bg-black/0">
			<div className="absolute top-0 left-4 font-mono text-[10px] tracking-[0.2em] text-neutral-600 uppercase opacity-50">
